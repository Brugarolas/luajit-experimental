...
#define LJ_GC_ON_FREE_LIST 0x8 // If set this arena has new free elements and must be re-scanned
#define LJ_GC_SWEEP_DIRTY 0x10 // If set lazy sweeping knows this arena is dirty.
...
#define makewhite(x) ((x)->gch.gcflags = ((x)->gch.gcflags & (uint8_t)~LJ_GC_COLORS))
...
{
  GCobj *o = obj2gco(t);
  lj_assertG(isblack(g, o) && !checkdead(g, o), "bad object states for backward barrier");
...
#define lj_gc_anybarriert(L, t) { if (LJ_UNLIKELY(isblack(G(L), obj2gco(t)))) { lj_gc_barrierback(G(L), (t)); } }
#define lj_gc_barriert(L, t, tv) { if (tviswhite(G(L), tv) && isblack(G(L), obj2gco(t))) { lj_gc_barrierback(G(L), (t)); } }
#define lj_gc_objbarriert(L, t, o) { if (iswhite(G(L), obj2gco(o)) && isblack(G(L), obj2gco(t))) { lj_gc_barrierback(G(L), (t)); } }

/* Barrier for stores to any other object. TValue and GCobj variant. */
#define lj_gc_barrier(L, p, tv) { if (tviswhite(G(L), tv) && isblack(G(L), obj2gco(p))) lj_gc_barrierf(G(L), obj2gco(p), gcV(tv)); }
#define lj_gc_objbarrier(L, p, o) { if (iswhite(G(L), obj2gco(o)) && isblack(G(L), obj2gco(p))) lj_gc_barrierf(G(L), obj2gco(p), obj2gco(o)); }
...
#define lj_mem_reallocv(L, p, on, n, t) (t *)lj_mem_reallocblob(L, p, (on) * sizeof(t), (MSize)((n) * sizeof(t)))
...
#define lj_gc_markblob(L, blob, size) G(L)->gc.bloblist_usage[gcablob(blob)->id] += size

typedef uint64_t bitmap_t;
#define WORD_BITS 64
#define SIMD_BITS _simd_bits
...
#define WORDS_FOR_TYPE(type) (((ELEMENTS_MAX(type) + SIMD_BITS - 1) / SIMD_BITS) * (SIMD_BITS / WORD_BITS))
#define WORDS_FOR_SZ(sz) ((((ARENA_SIZE / sz) + SIMD_BITS - 1) / SIMD_BITS) * 4)
#define SIMD_WORDS_FOR_TYPE(type) ((ELEMENTS_MAX(type) + SIMD_BITS - 1) / SIMD_BITS)
#define SIMD_MULTIPLIER (sizeof(_simd_default_type) / sizeof(bitmap_t))
#define WORDS_FOR_TYPE_UNROUNDED(type) ((ELEMENTS_MAX(type) + WORD_BITS - 1) / WORD_BITS)
#define HIGH_ELEMENTS_OCCUPIED(type) ((ELEMENTS_MAX(type)) & (WORD_BITS - 1))
#define ELEMENTS_OCCUPIED(hdr, type) ((sizeof(hdr) + sizeof(type) - 1) / sizeof(type))
...
#define FREE_HIGH(type) (~0ull >> HIGH_ELEMENTS_OCCUPIED(type) ? (WORD_BITS - HIGH_ELEMENTS_OCCUPIED(type)) : 1)
...
#define strtab_primary(g, id) &mref((g)->str.tab, StrTab)[((id) >> 4) & 0x3FFFFF]
#define strtab_secondary(g, id) &mref((g)->str.secondary_list[(id) >> 13], GCAstrtab)->entries[((id) >> 4) & 0x1FF]
